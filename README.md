...
- Контекстное меню карточек: на десктопе — ПКМ/клавиша контекстного меню, на мобильных — удержание (long tap); позволяет быстро выставлять статусы по ключевым колонкам без переключения во вкладку таблицы.
...
- Изменение статусов через контекстное меню выполняется с оптимистичным обновлением и показывает прогресс; при ошибке значения автоматически откатываются и выводится уведомление.
